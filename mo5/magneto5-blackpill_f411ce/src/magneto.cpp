...
K7::K7(uint32_t s) {
	max_size = s<<10; // max_size = s * 1024 = size in bytes
	cur_size = position = 0;
	tape = (byte*)malloc(max_size);
}
...
static inline int32_t min_int32_t(int32_t a, int32_t b) {
	if(a < b) return a;
	else return b;
}
...
int32_t K7::add(const byte *src, uint32_t size) {
	if(cur_size < max_size) {
		int32_t s = min_int32_t(size, max_size - cur_size);
		for(uint32_t i = 0; i < s; i++)
			tape[cur_size + i] = src[i];
		cur_size += size;
		return size;
	}
	return -1;
...
int32_t K7::read(byte *dst, uint32_t size) {
	if(position < cur_size) {
		int32_t s = min_int32_t(size, cur_size - position);
		for(uint32_t i = 0; i < s; i++)
			dst[i] = tape[position + i];
		position += s;
		return s;
	}
	return 0;
