...
// Déclaration locale des fonction du module
...
void Sortdirectory(char *path, char *extension) {
    ...
    while ((entry = readdir(dir))) {
        ...
        if (i < j)
            continue;
        if (strcasecmp(dirlist[dircount] + i - j, extension))
            continue;
        if (++dircount >= DIRLIST_NMAX)
            break;
    }
    closedir(dir);
    // strcpy(dirlist[dircount++], msg[language][39]); // [d�charger]
...
void Drawtextbox(SDL_Surface *surf, char *string, SDL_Rect rect, int c) {
    ...
    if (c > 9) {
        c -= 10;
        TTF_SetFontStyle(font11, TTF_STYLE_BOLD);
    }
    switch (c) {
    case 0:
        boxcolor = gris;
...
// Draw message box ////////////////////////////////////////////////////////////
void Drawmessagebox(char *titre, char *text1[], char *text2[])
// text1 est affich� sur fond gris et text2 sur fond blanc
...
void Drawk7index() {
    ...
    index[0] = 0;
    if (fk7 != NULL)
        sprintf(index, "%03d/%03d", k7index, k7indexmax);
    rect.x = 2;
    rect.w = 52;
    rect.y = 2;
    rect.h = 15;
...
void Drawstatusbar() {
    ...
    rect.x = 210;
    Drawtextbox(statusbar, fdname, rect, 1);
    rect.x = 362;
    Drawtextbox(statusbar, memoname, rect, 1);

    // boutons
    for (i = 0; i < STATUSBUTTON_MAX; i++) {
        rect.x = statusbutton[i].x;
        if (rect.x < 0)
            rect.x += xclient;
        rect.y = statusbutton[i].y;
        if (rect.y < 0)
            rect.y += yclient;
...
// Init status bar ////////////////////////////////////////////////////////////
void Initstatusbar() {
    statusbar = SDL_CreateRGBSurface(FLAGS, 2048, ystatus, 32, rmask, gmask,
                                     bmask, amask);
    if (statusbar == NULL)
        SDL_error(14);
    Drawstatusbar();
...
void Createbox() {
    ...
    dialogbox = SDL_CreateRGBSurface(FLAGS, dialogrect.w, dialogrect.h, 32,
    ...
    rect.w = 1;
    rect.y = 0;
    rect.h = dialogbox->h - 1;
    SDL_FillRect(dialogbox, &rect, blanc);
    // bordure bas noire
    rect.x = 1;
    rect.w = dialogbox->w - 2;
    rect.y = dialogbox->h - 2;
    rect.h = 1;
    ...
    rect.x = dialogbox->w - 2;
    rect.w = 1;
    rect.y = 1;
    rect.h = dialogbox->h - 3;
    SDL_FillRect(dialogbox, &rect, noir);
...
void Createdialogbox(int w, int h) {
    ...
    dialogrect.x = 10;
    dialogrect.w = w;
    dialogrect.y = ystatus + 10;
    dialogrect.h = h;
    Createbox(w, h);
    // barre de titre bleue
    rect.x = 4;
    rect.w = dialogbox->w - 8;
    rect.y = 3;
    rect.h = 18;
    ...
    r = SDL_BlitSurface(buttonsurface[closebutton.n][0], NULL, dialogbox,
                        &rect);
    if (r < 0)
        SDL_error(15);
}
...
void Statusclick() {
    ...
    for (i = 0; i < STATUSBUTTON_MAX; i++) {
        ...
        if (y < 0)
            y += yclient;
        if (xmouse > x)
            if (xmouse < (x + statusbutton[i].w))
                if (ymouse > y)
                    if (ymouse < (y + statusbutton[i].h))
                        break;
    }
    if (i >= STATUSBUTTON_MAX) {
        dialog = 0;
        return;
    }
    ...
    if (rect.y < 0)
        rect.y += yclient;
    r = SDL_BlitSurface(buttonsurface[statusbutton[i].n][1], NULL, screen,
                        &rect);
    if (r < 0) {
        SDL_error(16);
        return;
...
void Dialogclick() {
    ...
    if (xmouse > x)
        if (xmouse < (x + closebutton.w))
            if (ymouse > y)
                if (ymouse < (y + closebutton.h)) {
                    dialog = 0;
                    return;
                }
    ...
    if (dialog == 2)
        Optionclick(); // boite de dialogue Parametres
    if (dialog == 3)
        Keyclick(); // boite de dialogue Clavier
    if (dialog == 4)
        Joyclick(); // boite de dialogue Manettes
}
...
void Mouseclick() {
    ...
    if (dialog > 0)
        if (xmouse > dialogrect.x)
            if (xmouse < (dialogrect.x + dialogrect.w))
                if (ymouse > dialogrect.y)
                    if (ymouse < (dialogrect.y + dialogrect.h)) {
                        if (dialog > 999)
                            Menuclick();
                        else
                            Dialogclick();
...
void Drawmenubox() {
    ...
    int i;
    char string[50];
    dialogrect.x = xclient - 102;
    dialogrect.w = 100;
    dialogrect.y = ystatus;
    dialogrect.h = 3 * 16 + 10;
    Createbox();
    rect.x = 10;
    rect.w = 80;
    rect.y = 4;
...
void Drawdirlistbox(int n) {
    ...
    switch (n) {
    case 0:
        Sortdirectory("software", ".k7");
        break;
    case 1:
        Sortdirectory("software", ".fd");
        break;
    case 2:
        Sortdirectory("software", ".rom");
        break;
    default:
        return;
    }
    ...
    if (dirmax > dircount)
        dirmax = dircount;
    dialogrect.x = statusbutton[n].x;
    dialogrect.w = 200;
    dialogrect.y = ystatus;
    dialogrect.h = 16 * (dirmax - dirmin) + 26;
    if ((dirmax < dircount) || (dirmin > 0))
        dialogrect.h += 16; //[suite]/[debut]
    Createbox();
    rect.x = 10;
    rect.w = dialogrect.w - 12;
    rect.y = 4;
    rect.h = 14;
...
void Menuclick() {
    ...
    if (n == 3) // menu options
    {
        if (ymouse < (ystatus + 20)) {
            Options();
            return;
        }
        if (ymouse < (ystatus + 36)) {
            Clavier();
            return;
        }
        if (ymouse < (ystatus + 52)) {
            Manettes();
            return;
        }
        return;
