...
unsigned char cursor[] = {
    128, 0,   192, 0,   160, 0,   144, 0,   136, 0,   132, 0,   130, 0,
    129, 0,   128, 128, 128, 64,  128, 32,  129, 240, 137, 0,   152, 128,
    164, 128, 196, 64,  130, 64,  2,   32,  1,   32,  1,   16,  0,   176,
    0,   192, 0,   0,   0,   0,   128, 0,   192, 0,   224, 0,   240, 0,
    248, 0,   252, 0,   254, 0,   255, 0,   255, 128, 255, 192, 255, 224,
    255, 240, 255, 0,   231, 128, 199, 128, 131, 192, 3,   192, 3,   224,
    1,   224, 1,   240, 0,   240, 0,   192, 0,   0,   0,   0};
...
void About() {
    ...
    for (i = 0; i < 8; i++)
        text1[i] = msg[language][i + 2];
    text1[i] = "";
    text1[i] = "";
    for (i = 0; i < 5; i++)
        text2[i] = msg[language][i + 20];
    text2[i] = "";
...
void Playsound(void *udata, Uint8 *stream, int bufferlength) {
    ...
    // 45 cycles 6809 a 992250 Hz = un échantillon a 22050 Hz
    ...
    Testshiftkey(); // contournement bug SDL sur l'appui simultané SHIFT, ALT, CTRL
}
...
void Restorewindow()
{
 ...
 int penbutton, xclient, yclient, ystatus;
...
int main(int argc, char *argv[]) {
    ...
    SDL_WM_SetCaption(msg[language][0], NULL);   // titre fenetre
    SDL_SetCursor(
        SDL_CreateCursor(cursor, cursor + 48, 16, 24, 0, 0)); // curseur
