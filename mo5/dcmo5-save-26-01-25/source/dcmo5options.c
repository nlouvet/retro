...
void Drawoptionbox() {
    ...
    for (i = 0; i < OPTIONBUTTON_MAX; i++) {
        ...
        SDL_BlitSurface(buttonsurface[optionbutton[i].n][0], NULL, dialogbox,
                        &rect);
    }
    // texte
...
void Optionclick() {
    ...
    for (i = 0; i < OPTIONBUTTON_MAX; i++) {
        ...
        if (xmouse > x)
            if (xmouse < (x + optionbutton[i].w))
                if (ymouse > y)
                    if (ymouse < (y + optionbutton[i].h))
                        break;
    }
    if (i >= OPTIONBUTTON_MAX)
        return;
    // dessin du bouton enfonce
    rect.x = optionbutton[i].x;
    rect.y = optionbutton[i].y;
    SDL_BlitSurface(buttonsurface[optionbutton[i].n][1], NULL, dialogbox,
                    &rect);
    if (i == (OPTIONBUTTON_MAX - 1))
        Buttontext(2, rect.x, rect.y);
    Displayscreen();
    SDL_Delay(200);
    ...
    case 4:
        step = 10;
        frequency = frequency * 10 / 10; // faster
        if (frequency < 900)
            step = 100;
        if (frequency > 1099)
            step = 100;
        if (frequency > 1999)
            step = 1000;
        if (frequency < 9999 - step)
            frequency += step;
        break;
    case 5:
        step = 10;
        frequency = frequency * 10 / 10; // slower
        if (frequency < 901)
            step = 100;
        if (frequency > 1100)
            step = 100;
    ...
    case 6:
        framedelay /= 2;
        if (framedelay < 1)
            framedelay = 1; // more frames
        break;
    case 7:
        framedelay *= 2;
        if (framedelay > 64)
            framedelay = 64; // less frames
        break;
    case 8:
        k7protection = 0; // unprotect k7
        break;
    case 9:
        k7protection = 1; // protect k7
        break;
    case 10:
        fdprotection = 0; // unprotect fd
        break;
    case 11:
        fdprotection = 1; // protect k7
        break;
    case 12:
        Initdefault(); // restore default values
...
void Initoptions() {
    ...
    if (fread(string, 12, 1, fpi) != 1) goto Initoptions_error; // lecture identifiant
    if (strcmp("dcmo5ini-01", string) != 0) goto Initoptions_error;
    // initialisation des options
    if (fread(&language, 4, 1, fpi) != 1) goto Initoptions_error;  // langue
    if (fread(&frequency, 4, 1, fpi) != 1) goto Initoptions_error; // frequence 6809
    if (fread(&xclient, 4, 1, fpi) != 1) goto Initoptions_error;   // largeur ecran
    if (fread(&yclient, 4, 1, fpi) != 1) goto Initoptions_error;   // hauteur ecran
    if (fread(&framedelay, 4, 1, fpi) != 1) goto Initoptions_error;
    if (fread(&k7protection, 4, 1, fpi) != 1) goto Initoptions_error;
    if (fread(&fdprotection, 4, 1, fpi) != 1) goto Initoptions_error;
    // controle
    ...
    return;
Initoptions_error:
    fprintf(stderr, "Initoptions : error while reading the option file...\n");
}
