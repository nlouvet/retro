...
void Initbuttonbackground0(void *bitmap, int w, int h) {
    ...
    wline = 3 * w;
    for (i = 0; i < wline * h; i += 3)
        memcpy(bitmap + i, pfond, 3);
    // rangees verticales
    min = 2 * wline - 3;
    max = h * wline;
    for (i = min; i < max; i += wline)
        memcpy(bitmap + i, pnoir, 3);
    min = 2 * wline - 6;
    max = (h - 1) * wline;
    for (i = min; i < max; i += wline)
        memcpy(bitmap + i, pgris, 3);
    min = wline + 3;
    max = (h - 2) * wline;
    for (i = min; i < max; i += wline)
        memcpy(bitmap + i, pblanc, 3);
...
void Initkeysurfaces() {
    ...
    for (i = 0; i < MO5KEY_MAX; i++) {
        ...
        buttonsurface[i][0] = SDL_ConvertSurface(
            SDL_CreateRGBSurfaceFrom(bitmap, w, h, 24, w * 3, rmask, gmask,
                                     bmask, amask),
            screen->format, 0);
        if (buttonsurface[i][0] == NULL) {
            SDL_error(22);
            return;
        }
        // surface de fond de la touche enfoncee
        Initkeybackground1(bitmap, w, h);
        buttonsurface[i][1] = SDL_ConvertSurface(
            SDL_CreateRGBSurfaceFrom(bitmap, w, h, 24, w * 3, rmask, gmask,
                                     bmask, amask),
            screen->format, 0);
        if (buttonsurface[i][1] == NULL) {
            SDL_error(23);
            return;
        }
        ...
        if (mo5key[i].name[0] == '[') // dessin des bitmaps
        {
            ...
            case 'b':
                ...
                break;
            case 'g':
                picture = flechegauche;
                break;
            case 'd':
                picture = flechedroite;
                break;
            default:
                picture = flecheretour;
                break;
            }
        ...
        rect.x = 3;
        rect.y = 2;
        textbox = TTF_RenderUTF8_Blended(font9, mo5key[i].name, textnoir);
        if (textbox == NULL) {
            SDL_error(24);
            return;
        }
...
void Initbuttonsurfaces() {
    ...
    for (i = 0; i < OTHERBUTTON_MAX; i++) {
        ...
        for (j = 0; j < KEYBOARDBUTTON_MAX; j++)
            if (keyboardbutton[j].n == n) {
                keyboardbutton[j].w = w;
                keyboardbutton[j].h = h;
            }
        for (j = 0; j < JOYSTICKBUTTON_MAX; j++)
            if (joystickbutton[j].n == n) {
                joystickbutton[j].w = w;
                joystickbutton[j].h = h;
            }
        for (j = 0; j < STATUSBUTTON_MAX; j++)
            if (statusbutton[j].n == n) {
                statusbutton[j].w = w;
                statusbutton[j].h = h;
            }
        for (j = 0; j < OPTIONBUTTON_MAX; j++)
            if (optionbutton[j].n == n) {
                optionbutton[j].w = w;
                optionbutton[j].h = h;
            }
        if (closebutton.n == n) {
        ...
        buttonsurface[n][0] = SDL_ConvertSurface(
            SDL_CreateRGBSurfaceFrom(bitmap, w, h, 24, w * 3, rmask, gmask,
                                     bmask, amask),
            screen->format, 0);
        // surface du bouton enfonce
        Initbuttonbackground1(bitmap, w, h);
        buttonsurface[n][1] = SDL_ConvertSurface(
            SDL_CreateRGBSurfaceFrom(bitmap, w, h, 24, w * 3, rmask, gmask,
                                     bmask, amask),
            screen->format, 0);
        ...
        if (bouton[i].name[0] == '[') // dessin des bitmaps
        {
            ...
            case 'h':
                picture = flechehaut;
                imax = sizeof(flechehaut) / 4;
                break;
            case 'b':
                picture = flechebas;
                imax = sizeof(flechebas) / 4;
                break;
            case 'g':
                picture = flechegauche;
                imax = sizeof(flechegauche) / 4;
                break;
            case 'd':
                picture = flechedroite;
                imax = sizeof(flechedroite) / 4;
                break;
            case 'p':
                picture = boutonprev;
                imax = sizeof(boutonprev) / 4;
                break;
            case 'n':
                picture = boutonnext;
                imax = sizeof(boutonnext) / 4;
                break;
            default:
                picture = boutoncroix;
                imax = sizeof(boutoncroix) / 4;
                break;
            }
        ...
        rect.x = 4;
        rect.y = 1;
        textbox = TTF_RenderUTF8_Blended(font11, bouton[i].name, textnoir);
        if (textbox == NULL) {
            SDL_error(25);
            return;
        }
        ...
        if (SDL_BlitSurface(textbox, NULL, buttonsurface[n][0], &rect) < 0)
            SDL_error(26);
        // SDL_BlitSurface(textbox, NULL, buttonsurface[n][1], &rect);
        if (SDL_BlitSurface(textbox, NULL, buttonsurface[n][1], &rect) < 0)
            SDL_error(27);
    }
}
