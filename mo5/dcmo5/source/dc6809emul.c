...
// Par rapport au source initial, j'effectue notamment les
// transformations suivantes dans le code :
// char -> int8_t
// short -> int16_t
// unsigned short -> uint16_t
// En effet, les variables de types char étaient utilisées comme
// des variables signées. Or, un char peut être signé ou non d'après
// les standards, et cela peut poser des problèmes de portage
// (ex : Raspberry Pi). Et comme ça, il n'y a pas d'ambiguïté...

// processor registers
int CC = 0x10;                  // condition code
int16_t PC, D, X, Y, U, S, DA, W; // 6809 two byte registers + W work register

// ponters to processor registers
int8_t *PCHp, *PCLp, *Ap, *Bp; // pointers to register bytes
int8_t *XHp, *XLp, *YHp, *YLp; // pointers to register bytes
int8_t *UHp, *ULp, *SHp, *SLp; // pointers to register bytes
int8_t *DPp, *DDp, *WHp, *WLp; // pointers to register bytes

static unsigned int n; // cycle count
...
void Init6809registerpointers() {
    ...
    int8_t *c = (int8_t *)&i; // left byte of integer i

    PCHp = PCLp = (int8_t *)&PC; // PC msb & lsb
    Ap = Bp = (int8_t *)&D;      // D msb & lsb
    XHp = XLp = (int8_t *)&X;    // X msb & lsb
    YHp = YLp = (int8_t *)&Y;    // Y msb & lsb
    UHp = ULp = (int8_t *)&U;    // U msb & lsb
    SHp = SLp = (int8_t *)&S;    // S msb & lsb
    DPp = DDp = (int8_t *)&DA;   // DP msb & lsb
    WHp = WLp = (int8_t *)&W;    // W msb & lsb
...
void Addc(int8_t *r, int8_t c) {
    int i = *r + c;
    CC &= 0xd0;
    if (((*r & 0x0f) + (c & 0x0f)) & 0x10)
        CC |= H1;
    if (((*r & 0xff) + (c & 0xff)) & 0x100)
        CC |= C1;
    *r = i & 0xff;
    if (*r != i)
        CC |= V1;
...
void Adc(int8_t *r, int8_t c) {
    int carry = (CC & C1);
    int i = *r + c + carry;
    CC &= 0xd0;
    if (((*r & 0x0f) + (c & 0x0f) + carry) & 0x10)
        CC |= H1;
    if (((*r & 0xff) + (c & 0xff) + carry) & 0x100)
        CC |= C1;
    *r = i & 0xff;
    if (*r != i)
        CC |= V1;
...
void Addw(int16_t *r, int16_t w) {
    int i = *r + w;
    CC &= 0xf0;
    if (((*r & 0xffff) + (w & 0xffff)) & 0xf0000)
        CC |= C1;
    *r = i & 0xffff;
    if (*r != i)
        CC |= V1;
...
void Subw(int16_t *r, int16_t w) {
    int i = *r - w;
    CC &= 0xf0;
    if (((*r & 0xffff) - (w & 0xffff)) & 0x10000)
        CC |= C1;
    *r = i & 0xffff;
    if (*r != i)
        CC |= V1;
...
void Daa() {
    int i = A & 0xff;
    if ((CC & H1) || ((i & 0x00f) > 0x09))
        i += 0x06;
    if ((CC & C1) || ((i & 0x1f0) > 0x90))
        i += 0x60;
    A = i & 0xff;
    i = (i >> 1 & 0xff) | (CC << 7);
    CC &= 0xf0;
...
void Cmpw(int16_t *reg, int16_t w) {
    int16_t r = *reg;
    int i = *reg - w;
    CC &= 0xf0;
    if (((r & 0xffff) - (w & 0xffff)) & 0x10000)
...
// - 0 when operation code is illegal
// In both cases, *pcode receives the opcode that was formed
unsigned int Run6809(uint16_t *pcode) {
    uint16_t precode, code;
    ...
    *pcode = code;
    ...
    case 0x03:
        DIR;
        Mputc(DA, Com(Mgetc(DA)));
        return 6; /* COM  /$ */
    case 0x04:
        DIR;
        Mputc(DA, Lsr(Mgetc(DA)));
        return 6; /* LSR  /$ */
    // case 0x05: Mputc(DA, Lsr(Mgetc(DA))); return 6;      /*undoc LSR  /$ */
    case 0x06:
        DIR;
        Mputc(DA, Ror(Mgetc(DA)));
        return 6; /* ROR  /$ */
    case 0x07:
        DIR;
        Mputc(DA, Asr(Mgetc(DA)));
        return 6; /* ASR  /$ */
    case 0x08:
        DIR;
        Mputc(DA, Asl(Mgetc(DA)));
        return 6; /* ASL  /$ */
    case 0x09:
        DIR;
        Mputc(DA, Rol(Mgetc(DA)));
        return 6; /* ROL  /$ */
    case 0x0a:
        DIR;
        Mputc(DA, Dec(Mgetc(DA)));
        return 6; /* DEC  /$ */
    case 0x0c:
        DIR;
        Mputc(DA, Inc(Mgetc(DA)));
        return 6; /* INC  /$ */
    case 0x0d:
        DIR;
        Tstc(Mgetc(DA));
    ...
    case 0x32:
        IND;
        S = W;
        return 4 + n; /*CC not set*/ /* LEAS    */
    case 0x33:
        IND;
        U = W;
        return 4 + n; /*CC not set*/ /* LEAU    */
    case 0x34:
        Pshs(Mgetc(PC++));
        return 5 + n; /* PSHS    */
    case 0x35:
