...
// memory
int8_t car[0x10000]; // espace cartouche 4x16K
int8_t ram[0xc000];  // ram 48K
int8_t port[0x40];   // ports d'entree/sortie

// pointers
int8_t *ramvideo; // pointeur couleurs ou formes
int8_t *ramuser;  // pointeur ram utilisateur fixe
int8_t *romsys;   // pointeur rom systeme
int8_t *rombank;  // pointeur banque rom ou cartouche
uint16_t romoffset;

// flags cartouche
int cartype;  // type de cartouche (0=simple 1=switch bank, 2=os-9)
int carflags; // bits 0,1,4=bank, 2=cart-enabled, 3=write-enabled
...
void MO5rombank() {
    if ((carflags & 4) == 0) {
        ...
        rombank = car;
        romoffset = 0xb000 - ((carflags & 0x03) << 14);
    }
}
...
void Switchmemo5bank(unsigned short a) {
    ...
    carflags = (carflags & 0x00fc) | (a & 0x0003);
    MO5rombank();
}

// Signaux de synchronisation ligne et trame /////////////////////////////////
int Iniln() { // 11 microsecondes - 41 microsecondes - 12 microsecondes
    if (videolinecycle < 23)
        return 0;
    else
        return 0x20;
}

int Initn() { // debut à 12 microsecondes ligne 56, fin à 51 microsecondes ligne 255
    if (videolinenumber < 56)
        return 0;
    if (videolinenumber > 255)
        return 0;
    if (videolinenumber == 56)
        if (videolinecycle < 24)
            return 0;
    if (videolinenumber == 255)
        if (videolinecycle > 62)
            return 0;
    return 0x80;
}
...
void Joysemul(int i, int state) {
    ...
    switch (i) {
    case 0:
        if (joysposition & 0x02)
            n = 0x01;
        break;
    case 1:
        if (joysposition & 0x01)
            n = 0x02;
        break;
    case 2:
        if (joysposition & 0x08)
            n = 0x04;
        break;
    case 3:
        if (joysposition & 0x04)
            n = 0x08;
        break;
    case 4:
        if (joysposition & 0x20)
            n = 0x10;
        break;
    case 5:
        if (joysposition & 0x10)
            n = 0x20;
        break;
    case 6:
        if (joysposition & 0x80)
            n = 0x40;
        break;
    case 7:
        if (joysposition & 0x40)
            n = 0x80;
        break;
    case 8:
        if (state)
...
void Initprog() {
    ...
    for (i = 0; i < 58; i++)
        touche[i] = 0x80; // touches relach�es
...
// Traitement des opcode spéciaux introduits dans dcmo5 pour simuler
// les entrees-sorties sur les périphériques hors-clavier du mo5.
// Retourne 1 si l'opcode a bien été reconnu, 0 s'il s'agit d'une
// instruction illégale.
int Entreesortie(int opcode) {
    switch (opcode) {
    case 0x14:
        Readsector();
        return 1; // lit secteur qd-fd
    case 0x15:
        Writesector();
        return 1; // ecrit secteur qd-fd
    case 0x18:
        Formatdisk();
        return 1; // formatage qd-fd
    case 0x41:
        Readbitk7();
        return 1; // lit bit cassette
    case 0x42:
        Readoctetk7();
        return 1; // lit octet cassette
    case 0x45:
        Writeoctetk7();
        return 1; // ecrit octet cassette
    case 0x4b:
        Readpenxy();
        return 1; // lit position crayon
    case 0x51:
        Imprime();
        return 1; // imprime un caractere
    default:
        return 0; // code op. invalide
    }
}
...
int Run(int n) { // n est le nombre de cycles a exécuter
    ...
    i = 0;
    while (i < n) {
        opcycles = Run6809(&opcode); // execution d'une instruction
        if (opcycles == 0) {
            // L'instruction n'est pas légale dans le jeux d'instruction
            // du 6809. Mais il est fort possible que ce soit un opcode
            // utilisé par dcmo5 pour simuler les opérations d'entrée-sortie.
            if (!Entreesortie(opcode)) {
                fprintf(stderr, "Run: illegal instruction...");
            }
            opcycles = 64;
        }
...
char Mgetc(uint16_t a) {
    ...
    case 0xa:
        switch (a) {
        case 0xa7c0:
            return port[0] | 0x80 | (penbutton << 5);
        case 0xa7c1:
            return port[1] | touche[(port[1] & 0xfe) >> 1];
        case 0xa7c2:
            return port[2];
        case 0xa7c3:
            return port[3] | ~Initn();
        case 0xa7cb:
            return (carflags & 0x3f) | ((carflags & 0x80) >> 1) |
                   ((carflags & 0x40) << 1);
        case 0xa7cc:
            return ((port[0x0e] & 4) ? joysposition : port[0x0c]);
        case 0xa7cd:
            return ((port[0x0f] & 4) ? joysaction | sound : port[0x0d]);
        case 0xa7ce:
            return 4;
        case 0xa7d8:
            return ~Initn(); // octet etat disquette
        case 0xa7e1:
            return 0xff; // zero provoque erreur 53 sur imprimante
        case 0xa7e6:
            return Iniln() << 1;
        case 0xa7e7:
            return Initn();
